...
- Tests: +7 across the three (recovery_ok yes/no; absent/error patterns + missing-local integration; SRA fastq-dump command). Full suite **93 passed**.
- Remaining open review items: (#4) contamination/background model for shared cross-host genera; (#5) minor cruft (hard-coded email in `fetch_fungal_markers.py`, unused `*.sig`).
- Status: Accepted (#1–#3 done).
